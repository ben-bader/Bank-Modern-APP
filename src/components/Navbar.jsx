...
import { logo, menu ,close} from '../assets/import';
      ...
      <ul className="list-none sm:flex hidden
        justify-end items-center flex-1">
        {
            navLinks.map((nav,index)=>(
                ...
                text-[16px] hover:text-white text-cyan-400 duration-300
                  ${index == navLinks.length ? 'mr-0' : 'ml-10'} `}>
                    <a href={`#${nav.id}`}>
                        {nav.title}
                    </a>
                </li>
       ...
       justify-end items-center '>
        ...
        mx-4 my-2 min-w-[140px] rounded-xl sidebar`}>
            <ul className="list-none flex flex-col
              justify-start items-center flex-1">
        {
            navLinks.map((nav,index)=>(
                <li key={nav.id}
                className={`font-poppins font-normal cursor-pointer 
                text-[16px] text-white my-3 hover:text-cyan-400 duration-300
